...
$galleries = [
    ...
    "remodeling" => [
        "title" => "Remodeling",
        // Live site currently shows 20 original remodeling images
        "count" => 20,
    ],
];
...
$pageTitle = $currentTitle . " | JR Elite Builders";

$remoteSiteBase = "https://www.jrelitebuilders.com";
...
function buildRemoteGalleryItems(string $siteBase, string $slug, int $count): array
{
    $items = [];
...
function buildRemodelingGalleryItems(string $remoteSiteBase): array
{
    // Keep the 20 original remodeling images from the live site
    $items = buildRemoteGalleryItems($remoteSiteBase, "remodeling", 20);

    // Then append every local image found in:
    // /assets/img/galleries/remodeling/
    $localDir = __DIR__ . "/assets/img/galleries/remodeling";
    ...
    foreach ($matches as $filePath) {
        ...
        // Skip numeric files 1.jpg through 20.jpg so we do not duplicate
        // the original remote remodeling gallery set.
        if (ctype_digit($nameOnly)) {
            $n = (int)$nameOnly;
            if ($n >= 1 && $n <= 20) {
                continue;
            }
        ...
        $src = "/assets/img/galleries/remodeling/" . rawurlencode($filename);

        $items[] = [
            "src" => $src,
            "thumb" => $src,
            "alt" => "Remodeling " . trim(str_replace(["-", "_"], " ", $nameOnly)),
            "caption" => "Remodeling - " . trim(str_replace(["-", "_"], " ", $nameOnly)),
        ];
    }
...
function buildGalleryItems(string $slug, int $count, string $remoteSiteBase): array
{
    if ($slug === "") {
        return [];
    }
    ...
    if ($slug === "remodeling") {
        return buildRemodelingGalleryItems($remoteSiteBase);
    }

    return buildRemoteGalleryItems($remoteSiteBase, $slug, $count);
}

$galleryItems = [];

if ($slug !== "") {
    $galleryItems = buildGalleryItems($slug, (int)$galleries[$slug]["count"], $remoteSiteBase);
}
